...
export default (state: TrentState, action: {payload: Tpoints})=>{
    ...
    state.bikeRentInfo.forEach(biketype=>{
        ...
        biketype.cards.forEach(card=>{
            ...
            if(state.activeBikeSize !== 'Все' && biketype.type !== state.activeBikeSize){cardCheck = false}
            if(state.activeBikeBrand !== 'Все' && card.brandName !== state.activeBikeBrand){cardCheck = false}
