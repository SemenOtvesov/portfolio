...
import paginationContent from "@componentModules/paginationContent";
...
import mainRouteMouseMove from "@events/mainRoute/mainRouteMouseMove";
import mainRouteClick from "@events/mainRoute/mainRouteClick";
export default()=>{
    ...
    const [trainingRenderFrames, setTrainingRenderFrames] = useState({start: 0, end: 10})
    ...
    useEffect(()=>{
        ...
        const paginations: NodeListOf<HTMLLIElement> = document.querySelectorAll('[data-pagination-key]')
        paginations.forEach(el=>{
            if(el.dataset.paginationKey){
                if(+el.dataset.paginationKey * 10 === trainingRenderFrames.end){
                    el.classList.add('active')
                }else{
        ...
        <div id="childrensInBody" className="main">
                <div id="mainRight" className="main__right">
                    ...
                    <ul className="main__right-list">
                        {mainRouteTraningListItems(user.trainingList, user.trainingListKeys, userUid, activeTraining, trainingRenderFrames)}
                    </ul>
                    {
                        user.trainingList ? user.trainingList?.length > 10 ?
                        <nav>
                            <ul onClick={mainRouteClick.bind(this, setTrainingRenderFrames, setPaginationArray, setRerender)} className="main__right-pagination">
                                {paginationContent(user.trainingList, paginationArray, setPaginationArray)}
                            </ul>
                        </nav>:"":""
                    }
